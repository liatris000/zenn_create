...
> **close が唯一の中止シグナル。** 返信もチェックも不要で、放置すれば承認になる。
> `⚠️ disclosure: careful` が付いていても同じで、止めたいなら close する。
...
- 本業(実勤務先)領域に踏み込んでいないか(対象は `business-profile/policies/disclosure-rules.md`)
...
- 実装をコードレベルで進める
- **記事本文を書く**(成果物リポジトリの公開より先。2026-08-18 に順序を入れ替えた)
- 同 PR に追記コミット(タイトル: `[Day 2/3 WIP] ...`)
- 成果物リポジトリを公開し、本文のプレースホルダを実 URL に置換する
- Chatwork に Day 2 完了通知(翌朝チェック依頼)

> **なぜ本文が先か**: 以前は成果物公開が先で、失敗すると本文を書かずに停止していた。
> 2026-08-18 に公開経路(`repository_dispatch`)が塞がれただけでその週の記事が丸ごと
> 消えたため、順序を入れ替えた。公開が失敗しても本文と `[Day 2/3 WIP]` タイトルは
> 確定させ、PR に未完了コメントを残して Day 3 が再試行する。

### Day 3(水曜朝、Routine C: zenn-day3)

- 火曜の PR を探す + Liatris のフィードバック確認
- フィードバック反映
- 成果物が未公開のまま渡ってきていれば再試行する(Step 6.3)
- サムネ生成
...
| 起動日 | 探す PR | 見つからない時の挙動 | 2 本以上あった時の挙動 |
|---|---|---|---|
| Day 1 (月) | (新規) | 新サイクル開始 | — |
| Day 2 (火) | PR タイトル `[Day 1/3 WIP]` を含む open PR | 何もせず終了 + Chatwork 通知 | **停止**。該当 PR を列挙して終了 |
| Day 3 (水) | PR タイトル `[Day 2/3 WIP]` を含む open PR | 何もせず終了 + Chatwork 通知 | **停止**。該当 PR を列挙して終了 |

2 本以上あるときに先頭 1 件を黙って取ると、どちらが選ばれたかが実行後に分からない。
2026-09-14 に、対話セッションと cloud routine が並行して Day 1 を再実行し、
同じ公開スロットに `[Day 1/3 WIP]` が 2 本開いた(#87 / #88)。

### 承認は「無反応」で表される ⚠️

**PR が open のまま = 題材は承認済み。** Day 2 / Day 3 はそのまま進める。

- PR 本文のチェックボックスや `⚠️ disclosure: careful` の表示を停止理由にしてはならない
- Liatris が止めたいときは **close する**。それが唯一の中止シグナル
- したがって Day 1 は PR 本文にチェックボックス(`- [ ]`)を置かない

> 2026-08 に Day 2 / Day 3 が「チェックボックス未チェック = 未承認」と解釈して停止し、
> その PR が open のまま残ったため Day 1 も新サイクルを開始できず、3 週にわたって
> サイクルが止まった。

### 前サイクルが詰まったときの Day 1 の挙動

Day 1 は「前週の PR がマージ済み」を前提にしない。前提にすると、詰まった PR が 1 本
あるだけで以後すべての週が空振りする。

| 前サイクル PR | Day 1 の挙動 |
|---|---|
| 無い(マージ / close 済み) | 通常どおり新サイクル開始 |
| `[Day 3/3 Ready for Review]` | **新サイクル開始**(レビュー待ちは正常な状態)。マージ督促を通知 |
| `[Day 1/3 WIP]` / `[Day 2/3 WIP]` | 前サイクルが停止しているとみなし **自動 close** してから新サイクル開始 |

公開スロットは「既存 `published_at` の最大 + 7 日」で算出するため、Ready for Review の
PR と新サイクルが並走しても公開日は衝突しない。
...
- 一般的な記述に統一する
- ❌ NG 例: 「(実在の事業名)の OCR 契約書自動化を題材に Day 1 着手」
