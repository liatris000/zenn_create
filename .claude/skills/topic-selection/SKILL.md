...
(下記「採用前の 2 つのゲート」)。③④ の測り方の規則は「需要と供給は同じ語で測る」を参照。
...
③ 需要シグナルと ④ 供給シグナルは、**同じ検索語**で測る。
違う語で測った結果を ⑤ のマトリクスに入れてはいけない。

手順:

1. ③ と ④ で実際に使った検索語を、それぞれ 1 行で記録する
2. ④ の語が ③ の語を絞り込んだもの(③ ⊃ ④)になっていたら、
   **③ の判定を ④ に持ち越さない**。④ の語で需要を測り直す
3. 測り直して言及がほぼ見つからなければ 需要: low とする
4. 記録した 2 つの語は PR 本文の「採用判定の根拠」に必ず併記する

> 広い語で需要を測り、狭い語で供給を測ると、どんな題材でも
> 「需要 mid × 供給 空白 = ★★」になる。★★ は自動採用されるので、
> **切り口を狭めるほど採用されやすくなる**。判定として壊れている。
>
> 2026-09-14 の PR #84 がこれ。需要は「提案書自動生成 / AI 叩き台生成」で測り、
> 供給は「公開情報のみを入力に、相手ごとの叩き台サイトを事前に用意する」で測っていた。
> 後者で需要を測り直すと言及が無く、正しくは 需要 low × 供給 空白 = △(自動採用しない)。
...
見分け方: その切り口の語で需要を測り直して(上記手順 2)、言及がゼロなら後者。
...
2. その 1 文と同じ困りごとを述べている**実在の投稿・記事・Issue を 3 件、URL つきで挙げる**

| 挙げられた件数 | 判定 |
|---|---|
| 3 件以上 | 通過。その 1 文が ③ の検索語になる |
| 0〜2 件 | **需要 low**。候補から外す |

> 「たぶん困っている人はいるはず」で通してはいけない。**URL が挙がらないこと自体が判定**。
> 読者がその問題を共有していない題材は、書いても「何を伝えたいか分からない」になる。
...
| ★★ が 1 つ以上、★★★ なし | ③ と ④ の検索語が**一致している場合に限り**、第1候補(★★)を自動採用。Day 1 を続行し、PR 本文に「自動採用された ★★ 候補のため、題材として弱い可能性あり」と注意書きを記載。<br>**検索語が一致していなければ自動採用しない**(「需要と供給は同じ語で測る」に戻って測り直し、それでも ★★ なら Day 1 を中止)。 |
...
需要の検索語: [③ で実際に検索した語]
供給の検索語: [④ で実際に検索した語](需要の検索語と一致していること)
ゲートA(読者がその問題を持っているか): 通過 / 根拠 URL: [3 件]
