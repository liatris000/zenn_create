...
| PR の状態 | 意味 |
|---|---|
| open のまま | **題材は承認済み**。そのまま Day 3 を進めて Ready for Review まで完了させる |
| close されている | その週は中止(上の検索でヒットしないので自動的にそうなる) |

**PR 本文のチェックボックスが未チェックであることを停止理由にしてはならない。**
`⚠️ disclosure: careful` の表示も同様で、Liatris がレビューするときの注意喚起であって
ルーティンのゲートではない。Day 3 の役割は「完成させて Liatris に渡す」ことであり、
最終的な公開可否の判断は ④ のマージで行われる。
...
2. Step 0 の環境変数復元に失敗した
3. サムネイル生成が 3 回試して失敗した
...
published: true   # ← Day 3 完成時に false から true へ反転する
published_at: "2026-08-27 07:00"  # Day 1 で予約したスロット。ここでは変更しない
...
```bash
# published を true に反転(published_at は Day 1 予約をそのまま維持)
python3 - "articles/${ARTICLE_SLUG}.md" <<'PY'
import sys, re
p = sys.argv[1]
c = open(p, encoding='utf-8').read()
c2 = re.sub(r'^published:\s*false\s*$', 'published: true', c, count=1, flags=re.M)
assert c2 != c, "published: false が見つからない(既に true の可能性)"
open(p, 'w', encoding='utf-8').write(c2)
print("published: true に反転した")
PY

# published_at が入っているか確認(Day 1 予約漏れの検出)
grep -qE '^published_at:[[:space:]]*"[0-9]' "articles/${ARTICLE_SLUG}.md" \
  || echo "⚠️ published_at が無い。Day 1 の予約漏れ。./scripts/next-publish-slot.sh で再予約し frontmatter に追記すること"
```
...
```bash
# 予約スロットを frontmatter から読み出して報告に含める
PUBLISHED_AT=$(awk -F'"' '/^published_at:/ {print $2; exit}' "articles/${ARTICLE_SLUG}.md")
```

```
Day 3 完了 (PR: ${PR_URL}, 文字数: ${WORD_COUNT}字, 予約公開: ${PUBLISHED_AT})
週末にチェックお願いします。
レビュー後にマージすると、予約済みスロット(${PUBLISHED_AT})に自動公開されます(週刊連載キュー)。
```
...
- `published_at`(Day 1 予約)を勝手に変更しない(週刊連載キューの順序が崩れる)
- main へ直 push しない
